...
def update_figure(candidate_names: dcc.Input,
    ...
    def monthly_compositor(input_dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'candidate_name' in input_dataframe.columns:
            ...
            composite_monthly_average = input_dataframe.resample('M', on='start_date')[['dem_model_prediction', 'rep_model_prediction']].mean().dropna()
            months = pd.date_range(start=input_dataframe['start_date'].min(), end=input_dataframe['start_date'].max(), freq='M')
